...
from datasetbuild.o2o_load import read_vocab, read_category, batch_iter, process_file, process_testfile, build_vocab
...
class TextCNN(object):
    ...
    def cnn(self):
        ...
        with tf.name_scope("cnn"):
            ...
            conv = tf.layers.conv1d(conv, self.config.num_filters, self.config.kernel_size, name='conv2')
...
test_dir = os.path.join(base_dir, 'test_new.csv')
vocab_dir = os.path.join(base_dir, 'vocab.txt')

save_dir = 'checkpoints/o2otextcnn'
...
def val():
    ...
    msg = 'val Loss: {0:>6.2}, val Acc: {1:>7.2%}'
    ...
    y_val_cls = np.argmax(y_val, 1)
    y_pred_cls = np.zeros(shape=len(x_val), dtype=np.int32)  # 保存预测结果
    for i in range(num_batch):  # 逐批次处理
        start_id = i * batch_size
        end_id = min((i + 1) * batch_size, data_len)
        feed_dict = {
            model.input_x: x_val[start_id:end_id],
            model.keep_prob: 1.0
        }
    ...
    print(metrics.classification_report(y_val_cls, y_pred_cls, target_names=categories))
...
def test():
    ...
    x_test, ids = process_testfile(test_dir, word_to_id, config.seq_length)
    ...
    with open('sub/cnn_sub.csv', 'w') as f:
        f.write('id,label\n')
        for i in range(len(y_pred_cls)):
            f.write('{},{}\n'.format(ids[i], y_pred_cls[i]))
